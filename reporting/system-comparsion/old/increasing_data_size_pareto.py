    ...
    "IncreasingDataSize":
     {
         "dataset_sizes": np.array([8000, 16000, 32000, 64000, 128000]),
          ...
          tensorsocket_label: {"costs": np.array([18, 36, 72, 144, 288]), "throughputs": np.array([973.9196926, 973.9196926, 973.9196926, 973.9196926, 973.9196926])}
    },
    # "IncreasingNumJobs":
    # {
...
for workload_name, workload_data in workload.items():
    ...
    plt.plot(xaxis_values, tensorsocket_costs, label=tensorsocket_label,
    ...
    plt.plot(xaxis_values, disdl_costs, label=disdl_label,
    ...
    for i, size in enumerate(xaxis_values):
        if i == 0:
            plt.annotate(f"{tensorsocket_throughputs[i]:.0f}\nsamples/s", (size, tensorsocket_costs[i]), textcoords="offset points", xytext=(0,10), ha='center', fontsize=10, color='blue')
            plt.annotate(f"{disdl_throughputs[i]:.0f}\nsamples/sec", (size, disdl_costs[i]), textcoords="offset points", xytext=(0,10), ha='center', fontsize=10, color='red')
        else:
            plt.annotate(f"{tensorsocket_throughputs[i]:.0f}", (size, tensorsocket_costs[i]), textcoords="offset points", xytext=(0,10), ha='center', fontsize=10, color='blue')
            plt.annotate(f"{disdl_throughputs[i]:.0f}", (size, disdl_costs[i]), textcoords="offset points", xytext=(0,10), ha='center', fontsize=10, color='red')
    ...
    # Labels and legend
    plt.xlabel("Dataset Size", fontsize=font_size)
    plt.ylabel("Cost ($)", fontsize=font_size)
    plt.xscale("log")
    plt.xticks(xaxis_values, [f"{size//1000}K" for size in xaxis_values])  # Use 'K' notation
