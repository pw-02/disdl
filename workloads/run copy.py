...
from baselines.coordl.coordl_dataset import CoorDLImageNetIterableDataset, CoorDLMSCOCODataset, CoorDLOpenImagesIterableDataset
...
def run_training_job(config: DictConfig, train_logger: CSVLogger, val_logger: CSVLogger):
    ...
    if config.dataloader.name == 'disdl':
        ...
        train_dataset, train_sampler = get_coordl_dataset(config)
        train_dataloader = DataLoader(
...
def get_tansform(dataset_location):
    ...
    elif 'cifar10' in dataset_location:
        train_transform = transforms.Compose([
            transforms.RandomCrop(32, padding=4),   # adds 4-pixel padding and crops back to 32×32
            transforms.RandomHorizontalFlip(),      # mirror images with 50% probability
            transforms.ToTensor(),
...
def get_tensorsocker_dataset(config: DictConfig):
    dataset_location = config.workload.s3_train_prefix
    if 'imagenet' in dataset_location or 'cifar10' in dataset_location:
...
def get_coordl_dataset(config: DictConfig):
    ...
    if 'imagenet' in dataset_location:
        ...
        train_dataset = CoorDLOpenImagesIterableDataset(
            job_id=config.job_id,
            dataset_location=dataset_location,
            batch_size=config.workload.batch_size,
            transform=get_tansform(dataset_location),
