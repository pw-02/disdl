...
def main(config: DictConfig):
    ...
    if dataloader == 'tensorsocket':
        ...
        producer_cmd = f"{python_cmd} workloads/run.py workload={workload} dataloader={dataloader} dataloader.mode=producer workload.model_architecture={models[0]} workload.batch_size={batch_size}"
    ...
    for idx, model in enumerate(models):
        ...
        if dataloader == 'disdl':
            run_cmd = f"CUDA_VISIBLE_DEVICES={idx} {python_cmd} workloads/run.py workload={workload} exp_id={expid} job_id={idx} dataloader={dataloader} log_dir={log_dir} workload.model_architecture={model} workload.max_training_time_sec={max_train_time_seconds} workload.batch_size={batch_size}"
        elif dataloader == 'tensorsocket' and not producer_only:
            run_cmd = f"CUDA_VISIBLE_DEVICES={idx} {python_cmd} workloads/run.py workload={workload} exp_id={expid} job_id={idx} dataloader={dataloader} log_dir={log_dir} workload.model_architecture={model} dataloader.mode=consumer workload.max_training_time_sec={max_train_time_seconds} workload.batch_size={batch_size}"
